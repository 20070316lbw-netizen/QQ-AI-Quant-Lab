...
def run_backtest(df, top_k=50, label_col="label_next_month"):
    ...
    # 1. 筛选每月 Top-K 标的 (向量化加速)
    top_k_df = df.sort_values(['date', 'pred_rank'], ascending=[True, False]).groupby('date').head(top_k)
    portfolio_ret = top_k_df.groupby('date')[label_col].mean()
